...
import { checkFeedbackExist, insertFeedback, updateFeedback } from "@/lib/actions/sql/_index";
...
const text = {
  ...
  already_exist_desc: "หากต้องการเสนอความคิดเห็นกับการตอบกลับนี้อีกรอบกดปุ่ม \'ยืนยัน\' หากไม่ต้องการกดปุ่ม \'ยกเลิก\' หรือ \'กากบาท\' ",
  success: "ตอบรับความคิดเห็นสำเร็จ",
  success_desc: "ขอบคุณที่แสดงความคิดเห็นสำหรับการตอบกลับนี้ เพื่อให้เราใช้ในการพัฒนาผลิตภัณฑ์ต่อไป",
    ...
    if(Object.keys(feedbackExist.result).length !== 0) {
      toast.warning(text.already_exist, {
        ...
        action: {
          label: "ยืนยัน",
          onClick: async() => {
            feedbackObj = {...feedbackObj, feedbackId: feedbackExist.result.data_array[0][0]}
            try {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <Textarea onChange={(e) => setFeedback(e.target.value)} disabled={isInserting}/>
        <Button
          variant="kbank"
